...
export default function ParkHeader({ park, pageTitle }) {
  const nav = [
    ...
    // {href: `/${park.parkCode}/photos/`, text: 'Photos', active: pageTitle === 'Photos'},
    {
      ...
      active: pageTitle === "Visitor Centers",
    },
    {
      href: `/${park.parkCode}/camping/`,
      text: "Camping",
      active: pageTitle === "Camping",
    },
    {
      href: `/${park.parkCode}/trails/`,
      text: "Trails",
      active: pageTitle === "Trails",
    },
    {
      href: `/${park.parkCode}/things-to-do/`,
      text: "Things To Do",
  ...
  return (
    <div key={park.parkCode} className="park-header">
      <h1 className="park-header__name h2">{park.name}</h1>
      <p className="park-header__location">
        ...
        <ul className="park-header__links">
          {nav.map((n) => (
            <li key={`park-header-link-${urlString(n.text)}`}>
              <Link
                className={`park-header__link ${
                  n.text === pageTitle ? "park-header__link--active" : ""
                }`}
                href={n.href}
              >
                {n.text}
